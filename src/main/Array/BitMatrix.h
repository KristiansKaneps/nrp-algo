...
namespace BitMatrix {
    ...
    class BitMatrix {
    public:
        explicit BitMatrix(const dimension_size_t xSize, const dimension_size_t ySize) : m_XSize(xSize),
            m_YSize(ySize),
            m_Array(xSize * ySize) { }

        [[nodiscard]] array_size_t // ReSharper disable once CppMemberFunctionMayBeStatic
        offsetX(const dimension_size_t y) const { // NOLINT(*-convert-member-functions-to-static)
            return y;
        }

        [[nodiscard]] array_size_t offsetY(const dimension_size_t x) const { return x * m_YSize; }
    ...
    class BitMatrix3D {
    public:
        explicit BitMatrix3D(const dimension_size_t xSize, const dimension_size_t ySize, const dimension_size_t zSize) :
            m_XSize(xSize),
            m_YSize(ySize),
            m_ZSize(zSize),
            m_Array(xSize * ySize * zSize) { }

        [[nodiscard]] array_size_t offsetX(const dimension_size_t y, const dimension_size_t z) const {
            return y * m_ZSize + z;
        }

        [[nodiscard]] array_size_t offsetY(const dimension_size_t x, const dimension_size_t z) const {
            return x * m_YSize * m_ZSize + z;
        }

        [[nodiscard]] array_size_t offsetZ(const dimension_size_t x, const dimension_size_t y) const {
            return x * m_YSize * m_ZSize + y * m_ZSize;
        }

        void assign(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z, const uint8_t value) {
            m_Array.assign(index(x, y, z), value);
        }

        void assign(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z, const int8_t value) {
            m_Array.assign(index(x, y, z), value);
        }

        void assign(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z,
                    const uint16_t value) { m_Array.assign(index(x, y, z), value); }

        void assign(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z, const int16_t value) {
            m_Array.assign(index(x, y, z), value);
        }

        void assign(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z,
                    const uint32_t value) { m_Array.assign(index(x, y, z), value); }

        void assign(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z, const int32_t value) {
            m_Array.assign(index(x, y, z), value);
        }

        void assign(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z,
                    const uint64_t value) { m_Array.assign(index(x, y, z), value); }

        void assign(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z, const int64_t value) {
            m_Array.assign(index(x, y, z), value);
        }

        void set(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z) {
            m_Array.set(index(x, y, z));
        }

        void clear(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z) {
            m_Array.clear(index(x, y, z));
        }

        [[nodiscard]] uint8_t get(const dimension_size_t x, const dimension_size_t y, const dimension_size_t z) const {
            return m_Array.get(index(x, y, z));
        }

        uint8_t operator[](const dimension_size_t x, const dimension_size_t y, const dimension_size_t z) const {
    ...
    protected:
        ...
        [[nodiscard]] constexpr array_size_t offset(const dimension_size_t x, const dimension_size_t y) const {
            assert(x < m_XSize && "X must be less than the X dimension size.");
            assert(y < m_YSize && "Y must be less than the Y dimension size.");
            return x * m_YSize * m_ZSize + y * m_ZSize;
        }
        ...
        [[nodiscard]] constexpr array_size_t index(const dimension_size_t x, const dimension_size_t y,
                                                   const dimension_size_t z) const {
            assert(z < m_ZSize && "Z must be less than the Z dimension size.");
            return offset(x, y) + z;
        }
    };

    BitMatrix createMatrix(array_size_t xSize, array_size_t ySize);
    BitMatrix3D createMatrix3D(array_size_t xSize, array_size_t ySize, array_size_t zSize);
}
